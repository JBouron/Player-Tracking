...
namespace tmd{
    ...
    void FeaturesExtractor::createHistogram(player_t *p) {
        ...
        const int pixel_count = p->original_image.rows*p->original_image.cols;
        float** range = new float*[1];
        range[0] = new float[2];
        range[0][0] = 0; range[0][1] = static_cast<float>(180);
